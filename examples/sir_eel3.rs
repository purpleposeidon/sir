...
fn ordered_tys(armory: &Armory) -> Vec<Ty> {
    let mut order = Vec::<Ty>::new();
    let mut seen = HashSet::<Ty>::new();
    for (ty, sword) in armory.swords.iter() {
        let mut put = |ty: Ty| {
            if seen.insert(ty) {
                order.push(ty);
            }
...
impl<W: io::Write> EncoderBuilder<W> {
    ...
    pub fn compile(mut self, overrider: impl Fn(&mut Self, Ty) -> bool) -> CC<Ctx<W, CW>> {
        for &ty in &ordered_tys(&self.armory) {
            if overrider(&mut self, ty) {
                continue;
            }
            let sword = self.armory.swords.get(&ty).unwrap();
            match &sword.item.body {
...
fn main() {
    ...
    armory.add::<HashMap<u8, bool>>();
    armory.add::<AT>();
    let armory = &armory.build();

    type W = Vec<u8>;
    let mut builder = EncoderBuilder::<W>::new(armory.clone());
    builder.add_prims();
    ...
    let mut builder = DecoderBuilder::<R>::new(armory.clone());
    builder.add_prims();
    let cc = builder.compile(|_, _| false);
    {
